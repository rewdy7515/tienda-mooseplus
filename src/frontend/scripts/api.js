...
export async function fetchVentaEntregaByVenta(idVenta) {
  await ensureServerSession();
  try {
    const ventaIdNum = Number(idVenta);
    if (!Number.isFinite(ventaIdNum) || ventaIdNum <= 0) {
    ...
    const res = await fetch(
      `${API_BASE}/api/ventas/entrega-por-venta?id_venta=${encodeURIComponent(ventaIdNum)}`,
      {
        credentials: "include",
      }
    );
    if (!res.ok) {
      ...
      console.error("ventas/entrega-por-venta response", res.status, message);
      return { error: message || "No se pudo cargar la venta.", status: res.status };
    }
    return await res.json();
  } catch (err) {
    console.error("fetchVentaEntregaByVenta error", err);
    return { error: err.message };
  }
}
